...
void kalman_estimation(float theta ,float distance ,int new_simulation,float *final_theta,float *final_distance )
{
    ros::NodeHandle n;
    ros::ServiceClient client_kalman;
    ros::ServiceClient client_kalman2;
    simulator::simulator_base srv;
    client_kalman = n.serviceClient<simulator::simulator_base>("simulator_kalman_serv"); //create the client
    client_kalman2 = n.serviceClient<simulator::simulator_base>("simulator_kalman_serv2");
    srv.request.x1 = next.robot_x;
    srv.request.y1 = next.robot_y;
    srv.request.orientation = next.robot_theta;
    srv.request.theta = theta;
    ...
    if (client_kalman.call(srv))
    {
        printf("%s\n","KALMAN CLINET SUCCESS" );
    }
    else
    {
        ROS_ERROR("Failed to call service simulator_kalman_serv");
    }

    if (client_kalman2.call(srv))
    {
        printf("%s\n","KALMAN2 CLINET SUCCESS" );
    }
    else
    {
        ROS_ERROR("Failed to call service simulator_kalman_serv");
...
int move_robot(double theta,double advance,float lidar_readings[512] )
{
    float final_distance,final_theta;
    check_collision(theta ,advance ,new_simulation,&final_theta,&final_distance);
 
    
    kalman_estimation(theta ,advance ,new_simulation,&final_theta,&final_distance);
    move_gui(final_theta ,final_distance ,&next,lidar_readings);
    kalman_estimation(theta ,advance ,false,&final_theta,&final_distance);
